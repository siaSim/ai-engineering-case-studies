...
아래 이미지는 CellGuard 팀 프로젝트에서 사용한 **Team-level System Architecture**입니다.

![CellGuard AI System Architecture](../assets/cellguard/system-architecture.png)

*Team-level system architecture — 프로젝트 전체의 데이터 수집·저장·모델 학습/실험·추론·서비스·배포 구성을 보여주는 팀 설계 자료입니다. 이 다이어그램 전체를 개인 구현 범위로 주장하지 않습니다.*

### My Contribution Scope

이 전체 구조 중 제가 Commit과 공개 코드로 확인 가능한 범위는 **Azure Custom Vision 기반 외관검사 연동, CT inference 결과의 서비스 통합, 검사 결과 저장·Report UI, Azure App Service/GitHub Actions 배포 흐름**입니다. 모델 학습 전체, Azure ML 전체 구성, 팀 인프라 전체를 개인 기여로 주장하지 않습니다.
